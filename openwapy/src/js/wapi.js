...
window.WAPI.getChatById0 = function (id) {
  let found = WAPI.getChat(id);
  if (found) {
    found = WAPI._serializeChatObj(found);
  } else {
...
window.WAPI.reply = async function (chatId, body, quotedMsg) {
    ...
    if(!chat) return false;
        ...
        if(quotedMsg) {
						quotedMsg["body"] = ""
            extras = {
                quotedParticipant: "972512170493@c.us" || quotedMsg.from,
                quotedStanzaID:quotedMsg.id.id
            };
        }
    // var tempMsg = Object.create(Store.Msg.models.filter(msg => msg.__x_isSentByMe && !msg.quotedMsg)[0]);
		var tempMsg = Object.create(quotedMsg);
    var newId = window.WAPI.getNewMessageId(chatId);
    var extend = {
        ack: 0,
        id: newId,
        ...
        isNewMsg: !0,
        type: quotedMsg.type,
				quotedRemoteJid: "972512170493-1612427003@g.us",
				mediaData: quotedMsg.mediaData,
        quotedMsg,
        body,
        ...extras
    };
...
window.WAPI.reply0 = async function (chatId, body, quotedMsg) {
  if (typeof quotedMsg !== "object") quotedMsg = Store.Msg.get(quotedMsg)
  var chat = Store.Chat.get(chatId);
  if (!chat) return false;
  let extras = {
